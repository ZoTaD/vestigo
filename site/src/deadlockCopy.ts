/**
 * La copia de Deadlock, en los dos idiomas (2026-09-25).
 *
 * Vivía en `i18n.ts` con la de todo el sitio: dos tercios del JS de
 * traducciones, y lo bajaba también quien sólo miraba Valheim. Ahora viaja con
 * el chunk de Deadlock (`areas.ts`). Las mismas reglas que `i18n.ts`: el inglés
 * manda y el español se tipa contra su forma.
 *
 * Las pantallas de Deadlock importan `useCopy` de acá y no de `i18n.ts`: da la
 * copia del sitio con `deadlock` adentro, así que `copy.deadlock.x` y
...
const EN = {
  deadlock: {
    ...
    builder: {
      ...
      modes: { build: "Final build", guide: "Buy guide" },
      modeHint: {
        build: "Clicking an item adds it to your 12 slots.",
        guide: "Clicking an item adds it to the selected category.",
      },
      addedTo: (item: string, cat: string) => `${item} added to ${cat}.`,
      stats: {
        title: "Hero stats",
        note: "The game's base numbers plus what each item always gives. Conditional effects are not counted.",
        labels: {
          bulletDamage: "Bullet damage",
          pellets: "Bullets per shot",
          dps: "DPS",
          magazineDamage: "Damage per magazine",
          clip: "Ammo",
          fireRate: "Shots per second",
          reload: "Reload time",
      ...
      editor: {
        title: "Build editor",
        lead: "Like the game's: categories with a name and a note, items in buying order. Drag cards to reorder them or move them between categories, and drag the corner to resize a box.",
        addCategory: "Add category",
        newCategory: "New category",
        seed: "Start from the measured build",
        namePlaceholder: "Category name",
        descPlaceholder: "A note for this category",
        moveUp: "Move up",
        moveDown: "Move down",
        remove: "Delete category",
        resize: "Drag to resize (width and height)",
        drag: "Drag to move the category",
        select: (cat: string) => `Add items to ${cat}`,
        emptyCategory: "Click items in the shop or drag them here.",
        removeFrom: (item: string, cat: string) => `Remove ${item} from ${cat}`,
        empty: "No categories yet. Add one, or start from the build people play.",
      },
      abilities: {
        title: "Ability point order",
        hint: "Click an ability to give it its next point. The diamond unlocks it; 1, 2 and 5 are the points each upgrade costs.",
        add: (name: string) => `Give a point to ${name}`,
        undo: "Undo",
        clear: "Clear",
        loadMeasured: "Load the measured order",
        unlock: "unlock",
        upgrade: (n: number) => `upgrade, ${n} pts`,
      },
...
const ES: typeof EN = {
  deadlock: {
    ...
    builder: {
      ...
      modes: { build: "Build final", guide: "Guía de compra" },
      modeHint: {
        build: "Hacer clic en un objeto lo suma a tus 12 casillas.",
        guide: "Hacer clic en un objeto lo suma a la categoría seleccionada.",
      },
      addedTo: (item, cat) => `${item} agregado a ${cat}.`,
      stats: {
        title: "Estadísticas del héroe",
        note: "Los números base del juego más lo que cada objeto da siempre. Los efectos condicionales no se cuentan.",
        labels: {
          bulletDamage: "Daño por bala",
          pellets: "Balas por disparo",
          dps: "DPS",
          magazineDamage: "Daño por cargador",
          clip: "Munición",
          fireRate: "Disparos por segundo",
          reload: "Tiempo de recarga",
          weaponDamage: "Daño de arma",
          bulletVelocity: "Velocidad de bala",
          bulletLifesteal: "Robo de vida de balas",
          lightMelee: "Cuerpo a cuerpo ligero",
          heavyMelee: "Cuerpo a cuerpo pesado",
          health: "Vida",
          healthRegen: "Regeneración de vida",
          outOfCombatRegen: "Regeneración fuera de combate",
          bulletResist: "Resistencia a balas",
          spiritResist: "Resistencia espiritual",
          meleeResist: "Resistencia cuerpo a cuerpo",
          debuffResist: "Resistencia a desventajas",
          moveSpeed: "Velocidad de movimiento",
          sprintSpeed: "Velocidad de carrera",
          stamina: "Aguante",
          spiritPower: "Poder espiritual",
          cooldownReduction: "Reducción de enfriamiento",
          abilityRange: "Alcance de habilidades",
          abilityDuration: "Duración de habilidades",
          spiritLifesteal: "Robo de vida espiritual",
        },
      },
      editor: {
        title: "Editor de la build",
        lead: "Como el del juego: categorías con nombre y una nota, y los objetos en el orden en que se compran. Arrastrá las tarjetas para reordenarlas o pasarlas de categoría, y la esquina para cambiar el tamaño del recuadro.",
        addCategory: "Añadir categoría",
        newCategory: "Nueva categoría",
        seed: "Empezar de la build medida",
        namePlaceholder: "Nombre de la categoría",
        descPlaceholder: "Una nota para esta categoría",
        moveUp: "Subir",
        moveDown: "Bajar",
        remove: "Borrar categoría",
        resize: "Arrastrá para cambiar el ancho y el alto",
        drag: "Arrastrá para mover la categoría",
        select: (cat) => `Agregar objetos a ${cat}`,
        emptyCategory: "Hacé clic en objetos de la tienda o arrastralos acá.",
        removeFrom: (item, cat) => `Quitar ${item} de ${cat}`,
        empty: "Todavía no hay categorías. Añadí una, o empezá de la build que se juega.",
      },
      abilities: {
        title: "Orden de los puntos de habilidad",
        hint: "Hacé clic en una habilidad para darle su próximo punto. El rombo la desbloquea; 1, 2 y 5 son los puntos que cuesta cada mejora.",
        add: (name) => `Darle un punto a ${name}`,
        undo: "Deshacer",
        clear: "Borrar",
        loadMeasured: "Cargar el orden medido",
        unlock: "desbloqueo",
        upgrade: (n) => `mejora, ${n} pts`,
      },
...
export const DEADLOCK_COPY: Record<Lang, typeof EN> = { en: EN, es: ES };
